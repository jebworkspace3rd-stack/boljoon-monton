...
function saveRecords(records) { localStorage.setItem(STORAGE_KEY, JSON.stringify(records)); renderAllTabs(); }
...
function saveScoringResult(fightId, resultType) {
  ...
  renderScoringTab(); renderScheduleTab(); renderOverallResults(); renderParadaTab();
}
...
function getAllEntryNames() { return [...new Set(loadRecords().flatMap(r => [r.left.entryName, r.right.entryName]))]; }
...
function renderFightTab() {
  ...
  container.innerHTML = `<div class="arena-container"><div class="section-title">🔥 REGISTER FIGHT</div>
    ...
    <div class="fight-grid"><div class="fighter-card">
        <div class="entry-label">🐓 ENTRY NAME</div><input type="text" id="entryNameLeft" class="input-field" autocomplete="off"><div id="leftSuggestions" class="suggestions-list" style="display:none;"></div>
          <div class="entry-label">🎽LEG BAND #</div><input type="number" id="bandLeft" class="input-field">
            <div class="entry-label">💰 PARADA</div><input type="number" id="paradaLeft" class="input-field" value="0" min="0" step="100"></div>
    
    <div class="vs-divider">VS</div>

      <div class="fighter-card">
            <div class="entry-label">🐓 ENTRY NAME</div><input type="text" id="entryNameRight" class="input-field" autocomplete="off"><div id="rightSuggestions" class="suggestions-list" style="display:none;"></div>
          <div class="entry-label">🎽LEG BAND #</div><input type="number" id="bandRight" class="input-field">
        <div class="entry-label">💰 PARADA</div><input type="number" id="paradaRight" class="input-field" value="0" min="0" step="100"></div></div>
      <div style="display:flex; justify-content:center;"><button id="submitFightBtn" class="btn btn-primary">✔️ SUBMIT FIGHT</button></div>
    <div class="registered-entries">
      <div style="
        margin-bottom:10px; 
        font-weight:bold;
        color:#f5d97a;
      ...
      <div id="registeredEntriesList" class="entries-list"></div></div></div>`;
...
function renderRegisteredEntries() {
  ...
  let html = "";
  records.slice(0, 20).forEach(rec => { html += `
  <div class="entry-tag"> ${escapeHtml(rec.left.entryName)} | ${rec.left.bandNumber} | ${formatParada(rec.left.parada)}
  </div><div class="entry-tag"> ${escapeHtml(rec.right.entryName)} | ${rec.right.bandNumber} | ${formatParada(rec.right.parada)}</div>`; });
...
function renderParadaTab() {
  ...
  container.innerHTML = `<div class="arena-container">
    
    <div class="section-title"> PARADA MANAGEMENT</div>
    <div class="search-bar" style="position:relative;">
      <input type="text" id="paradaSearchInput" class="search-input" placeholder="Search entry..." autocomplete="off">
        <div id="paradaSuggestions" class="suggestions-list" style="display:none;"></div>
        <button id="paradaSearchBtn" class="btn">Search</button>
        <button id="paradaClearBtn" class="btn">Clear</button></div>
  ...
  if (!filtered.length) { tbody.innerHTML = "<tr><td colspan='8'>No matches found</td></tr>"; } else {
    ...
    filtered.forEach((rec, idx) => {
      ...
      const leftParadaStatus = rec.left.parada > rec.right.parada ? 'MERON' : (rec.left.parada < rec.right.parada ? 'WALA' : 'N/A');
      const rightParadaStatus = rec.right.parada > rec.left.parada ? 'MERON' : (rec.right.parada < rec.left.parada ? 'WALA' : 'N/A');
...
window.printSingle = function(id, side) {
  ...
  <body>
    <div class="receipt">
      <div class="header">
        <h2>BOLJOON COCKPIT MONTON</h2>
        <p>Parada Slip</p>
      </div>
      <div class="content">
        <div class="entry">
          <span class="label">Entry:</span> ${escapeHtml(e.entryName)}
        </div>
        <div class="entry">
          <span class="label">Leg Band#:</span> ${e.bandNumber}
        </div>
        <div class="entry">
          <span class="label">Parada:</span> ${formatParada(e.parada)}
        </div>
        <div class="entry">
...
function renderScheduleTab() {
  ...
  container.innerHTML = `<div class="arena-container">
    <div class="section-title">📅 FIGHTING SCHEDULE </div>
    <div class="search-bar" style="position:relative;">
      <input type="text" id="scheduleSearchInput" class="search-input" placeholder="Search entry..." autocomplete="off">
      <div id="scheduleSuggestions" class="suggestions-list" style="display:none;"></div>
      <button id="scheduleSearchBtn" class="btn">Search</button>
      <button id="scheduleClearBtn" class="btn">Clear</button>
    </div>
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
  ...
  if (!filtered.length) { tbody.innerHTML = "<tr><td colspan='8'>No matches found</td></tr>"; } else {
    ...
    filtered.forEach((rec, idx) => {
      ...
      html += `<tr class="${rowClass}"><td rowspan="2"><span class="fight-id-badge">#${idx+1}</span></td><td rowspan="2">${rec.submittedAt}</td>
        <td style="color:#dc3545; font-weight:bold;">MERON</td>
      ...
      <tr class="${rowClass}">
        <td style="color:#0d6efd; font-weight:bold;">WALA</td>
        ...
        <head>
          ...
          <body>
              <div class="receipt">
                <div class="header">
                  <h2>
                      BOLJOON COCKPIT MONTON
                  </h2>
                  <p> 
                      Fight Schedule Slip
                  </p>
                </div>
              <div class="datetime"> ${new Date().toLocaleString()}
              </div><div class="fight-number">FIGHT #${loadRecords().findIndex(r=>r.id==id)+1}</div>
              <div class="difference">DIFFERENCE: ${formatParada(ranked.difference)}</div>
              
              <div class="meron-section">
                <div class="entry-name">MERON</div>
                <div class="entry-name"> ${escapeHtml(ranked.higher.entryName)}</div>
                <div class="parada-amount">PARADA: ${formatParada(ranked.higherParada)}</div>
              </div>
              
              <div class="vs-spacer">VS</div>
              
              <div class="wala-section">
                <div class="entry-name">WALA</div>
                <div class="entry-name"> ${escapeHtml(ranked.lower.entryName)}</div>
                <div class="parada-amount">PARADA: ${formatParada(ranked.lowerParada)}
              </div>
              
                  </div><div class="result">RESULT: ${resultText}</div>
                  
                <div class="footer">
                <hr><p>Printed: ${new Date().toLocaleString()}</p>
                </div></div></body></html>`);
...
function renderScoringTab() {
  ...
  container.innerHTML = `<div class="arena-container"><div class="section-title">🏆 SCORING OFFICIAL</div>
    ...
    <div class="table-wrapper"><table class="data-table">
      ...
      <tbody id="scoringTableBody"></tbody></table>
    </div>
  </div>`;
  ...
  if (!filtered.length) { tbody.innerHTML = "<tr><td colspan='5'>No fights registered</td></tr>"; } else {
    ...
    filtered.forEach((rec, idx) => {
      ...
      html += `<tr class="${rowClass}">
        ...
        <td><div><span class="rank-badge-higher">MERON</span> <span ${higherHighlight}> ${escapeHtml(ranked.higher.entryName)}</span> (${formatParada(ranked.higherParada)})</div>
          <div style="
            
            margin: 5px 0;"> VS. </div> 
        <div><span class="rank-badge-lower">WALA</span> <span ${lowerHighlight}> ${escapeHtml(ranked.lower.entryName)}</span> (${formatParada(ranked.lowerParada)})</div></td>
        <td>${display}</td>
        <td><select class="win-loss-select" id="score_select_${rec.id}" onchange="updateScoringFromTab('${rec.id}', this.value)" ${dropdownDisabled}>
          ...
          <option value="HIGHER_WIN" ${selectedHigher}>🏆 MERON WINS </option>
          <option value="LOWER_WIN" ${selectedLower}>🏆 WALA WINS </option>
...
function renderOverallResults() {
  ...
  container.innerHTML = `<div class="arena-container">
    ...
    <div class="search-bar" style="position:relative;">
        <input type="text" id="overallSearchInput" class="search-input" placeholder="Search entry name..." autocomplete="off">
      <div id="overallSuggestions" class="suggestions-list" style="display:none;"></div>
        <button id="overallSearchBtn" class="btn">🔍 Search</button><button id="overallClearBtn" class="btn">🗑️ Clear</button></div>
        <div class="info-note">🏆 MERON = Higher wins | 🏆 WALA = Lower wins | 🚫 NO FIGHT = Did not take place | 🗑️Clear button to reset search</div>
        <div class="table-wrapper"><table class="data-table">
        <thead>
          < tr><th>Rank</th><th>Entry Name</th><th>WINS</th><th>LOSSES</th><th>DRAWS</th><th>CANCELS</th><th>NO FIGHT</th><th>Total</th></tr></thead><tbody id="overallBody"></tbody></table></div></div></div>`;
...
function renderSettingsTab() {
  ...
  document.getElementById("deleteAllBtn")?.addEventListener("click", () => { if (confirm("DELETE ALL FIGHTS?")) { localStorage.setItem(STORAGE_KEY, "[]"); localStorage.removeItem(SCORING_KEY); renderAllTabs(); } });
...
// INITIALIZATION
function renderAllTabs() { renderFightTab(); renderParadaTab(); renderScheduleTab(); renderScoringTab(); renderOverallResults(); renderSettingsTab(); renderPrintHistory(); }
